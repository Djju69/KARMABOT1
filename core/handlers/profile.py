...
async def on_my_cards(message: Message):
    ...
    try:
        ...
        if not cards:
            ...
            if len(cards) > 5:
                cards_text += f"• ... и еще {len(cards) - 5} карт\n"
            
            await message.answer(
                f"💳 <b>Мои карты</b>\n\n"
                f"📋 <b>Привязанные карты ({len(cards)}):</b>\n"
                f"{cards_text}\n"
                f"💡 <b>Как добавить карту:</b>\n"
                f"• Введите номер карты\n"
                f"• Отсканируйте QR-код\n"
                f"• Создайте виртуальную карту",
                parse_mode="HTML"
            )
    except Exception as e:
...
async def on_my_points(message: Message):
    ...
    try:
        ...
        await message.answer(
            f"💎 <b>Мои баллы</b>\n\n"
            f"💰 <b>Текущий баланс:</b> {points_balance} баллов\n\n"
            f"📊 <b>История начислений:</b>\n"
            f"{history_text}\n\n"
            f"🎯 <b>Как получить баллы:</b>\n"
            f"• Ежедневный вход: +5 баллов\n"
            f"• Привязка карты: +25 баллов\n"
            f"• Рефералы: +50 баллов",
            parse_mode="HTML"
        )
    except Exception as e:
