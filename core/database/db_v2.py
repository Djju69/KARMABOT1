...
class DatabaseServiceV2:
    ...
    def get_cards_by_category(self, category_slug: str, status: str = 'published', limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"ДИАГНОСТИКА: Ошибка при получении карточек для категории '{category_slug}': {e}")
            return []
...
def get_db():
    ...
    def get_card_photos(self, card_id: int) -> List[Dict]:
        """Get photos for a specific card"""
        try:
            with self.get_connection() as conn:
                cursor = conn.execute("""
                    SELECT photo_url, photo_file_id, display_order
                    FROM card_photos 
                    WHERE card_id = ?
