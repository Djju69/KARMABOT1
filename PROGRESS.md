...
- **Сервисы: 80%** 🔄
  - QR‑сервис: 85% (`web/routes_qr.py`; redeem в `web/routes_cabinet.py` L212-L304)
...
- **Тестирование: 60%** 🔄
  - Unit тесты: 60%
  - Integration: 60%
  - E2E: 50% (добавлен каркас `tests/e2e/test_user_journey.py`)
  - Покрытие кода: ~60-70% (по структуре набора тестов)
  - **Проблемы:**
    - Отсутствует запуск тестов локально без venv (ограничение окружения)
