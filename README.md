...
Запуск backend для Telegram WebApp (авторизация и профили):
...
Эндпоинты:
- `POST /auth/webapp` — принимает JSON `{ "initData": "<строка из Telegram WebApp>" }`, возвращает `{ token }` (JWT)
- `GET /auth/me` — заголовок `Authorization: Bearer <token>`, возвращает клеймы

Переменные окружения (см. `.env.example`):
- `JWT_SECRET` — секрет для подписи JWT (если пусто — dev-подпись "debug")
- `AUTH_WINDOW_SEC` — окно валидности `initData` (по умолчанию 300 сек)
- `WEBAPP_QR_URL` — URL, куда ведёт кнопка `/webapp` в боте
- `WEBAPP_ALLOWED_ORIGIN` — разрешённый Origin фронтенда (CORS)
- `CSP_ALLOWED_ORIGIN` — разрешённый Origin для CSP (connect-src и т.д.)
- `FASTAPI_ONLY` — запуск только FastAPI без обязательных BOT_TOKEN/ADMIN_ID (разработческий режим, dev-флаг)

> Примечание: доступ к WebApp в самом боте — только через `/webapp` (инлайн‑кнопка `WebAppInfo`).
