...
def validate_environment():
    ...
    # Проверяем переменные Odoo (опционально)
    ...
    if odoo_missing:
        logger.warning(f"⚠️ Odoo variables missing: {', '.join(odoo_missing)}")
        logger.warning("💡 Odoo integration will be disabled")
    else:
        logger.info("✅ Odoo variables found - integration enabled")
...
async def main():
    ...
    # Тест подключения к Odoo (если переменные настроены)
    try:
        from odoo_connector import test_odoo_connection
        if await test_odoo_connection():
            logger.info("✅ Odoo connection test passed")
        else:
            logger.warning("⚠️ Odoo connection test failed")
    except Exception as e:
        logger.warning(f"⚠️ Odoo connection test error: {e}")
