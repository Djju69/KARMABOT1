...
Система работает даже при отказе PostgreSQL или Supabase
"""
import asyncio
import json
import logging
...
class FaultTolerantService:
    ...
    def get_user_with_fallback(self, user_identifier: Union[int, str], platform: str) -> Optional[Dict]:
        ...
        try:
            ...
            if self.health_monitor.supabase_status:
                try:
                    # Заглушка для тестирования
                    supabase_profile = {'user_id': user_identifier, 'platform': platform, 'source': 'supabase'}
                    if supabase_profile:
                        # Кэшируем результат
            ...
            if self.health_monitor.postgresql_status:
                try:
                    # Заглушка для тестирования
                    main_user = {'user_id': user_identifier, 'platform': platform, 'source': 'postgresql'}
                    if main_user:
                        # Кэшируем результат
    ...
    def create_order_with_fallback(self, user_identifier: Union[int, str], order_data: Dict, platform: str) -> Optional[int]:
        ...
        try:
            ...
            if self.health_monitor.postgresql_status:
                try:
                    # Заглушка для тестирования - генерируем временный ID
    ...
    def get_loyalty_with_fallback(self, user_identifier: Union[int, str], platform: str) -> Dict:
        ...
        try:
            ...
            if self.health_monitor.supabase_status:
                try:
                    # Заглушка для тестирования
                    points = 100  # Тестовые баллы
                    cards = []  # Тестовые карты
